...
def bg(seed):
    ...
    for y in range(H):
        for x in range(W):
            ...
            px[x,y]=(min(255,int(9+glow+seed*2)),min(255,int(12+glow*.6+seed*2)),min(255,int(24+glow+seed*4)))
...
def card(d,box,fill=(22,29,53,245),outline=(100,130,255,160),radius=36,width=3): d.rounded_rectangle(box,radius=radius,fill=fill,outline=outline,width=width)

def header(d,title,sub=''):
    d.text((70,65),'AI TOOLS  •  @SKILLEDTECHIE',font=font(28,True),fill=(130,170,255,255))
    center(d,title,145,font(76,True),maxw=900)
    if sub: center(d,sub,250,font(32),fill=(205,212,235),maxw=880)
...
def product_ui(d,tool):
    ...
    d.rounded_rectangle((x,y,x+w,y+100),radius=42,fill=(31,40,70,255)); d.rectangle((x,y+58,x+w,y+100),fill=(31,40,70,255))
    for i,c in enumerate([(255,95,95),(255,205,70),(90,220,130)]): d.ellipse((x+35+i*35,y+34,x+55+i*35,y+54),fill=c)
    d.text((x+165,y+25),tool,font=font(38,True),fill=(255,255,255,255))
    card(d,(x+30,y+135,x+260,y+h-35),fill=(229,233,244,255),outline=(200,205,220,255),radius=24,width=2)
    d.text((x+55,y+180),'Sources',font=font(28,True),fill=(55,65,90))
    for i,t in enumerate(['Research.pdf','Notes.docx','Paper.pdf']):
        yy=y+245+i*75; d.rounded_rectangle((x+50,yy,x+230,yy+48),radius=12,fill=(255,255,255,255)); d.text((x+66,yy+10),t,font=font(18),fill=(65,75,100))
    cx=x+285
    d.text((cx+35,y+175),'Ask about your sources',font=font(29,True),fill=(45,55,80))
    d.rounded_rectangle((cx+30,y+245,x+w-55,y+345),radius=20,fill=(247,249,253,255),outline=(215,220,232,255),width=2)
    d.text((cx+55,y+275),'Summarize the key ideas',font=font(26),fill=(75,85,110))
    d.rounded_rectangle((cx+30,y+385,x+w-55,y+655),radius=24,fill=(232,236,246,255),outline=(205,212,228,255),width=2)
    d.text((cx+55,y+420),'AI ANSWER',font=font(26,True),fill=(65,95,180))
    answers=['Key ideas extracted from your files.','Answers stay grounded in your sources.','Ask follow-up questions instantly.']
    for i,t in enumerate(answers): d.text((cx+55,y+475+i*48),t,font=font(23),fill=(65,70,88))
    d.rounded_rectangle((cx+30,y+690,x+w-55,y+755),radius=18,fill=(65,105,230,255)); d.text((cx+205,y+706),'Ask another question',font=font(23,True),fill=(255,255,255))
...
def make_scenes(tool,hook):
    ...
    for i,(t,s) in enumerate(rows): y=690+i*260; d.ellipse((125,y,215,y+90),fill=(70,110,245,255)); d.text((150,y+23),str(i+1),font=font(30,True),fill=(255,255,255)); d.text((260,y+5),t,font=font(36,True),fill=(255,255,255)); d.text((260,y+65),s,font=font(25),fill=(190,200,225)); scenes.append(im)
    im=bg(5); d=ImageDraw.Draw(im,'RGBA'); d.ellipse((350,260,730,640),fill=(70,110,245,255)); center(d,'@',350,font(120,True)); center(d,'Want more AI tools that actually save time?',780,font(55,True),maxw=900); card(d,(150,1120,930,1290),fill=(255,255,255,255),outline=(255,255,255,255),radius=70); center(d,'FOLLOW @SKILLEDTECHIE',1155,font(42,True),fill=(25,35,65),maxw=700); center(d,'New tools. Real use cases. No fluff.',1380,font(32),fill=(200,210,235)); scenes.append(im)
    return scenes

def run(cmd): print('>',' '.join(map(str,cmd))); subprocess.run(cmd,check=True)
def probe(path): return float(subprocess.check_output(['ffprobe','-v','error','-show_entries','format=duration','-of','default=nw=1:nk=1',str(path)]).decode().strip())
...
def main():
    ...
    try: run(['edge-tts','--voice','en-IN-NeerjaNeural','--rate','+5%','--text',voice,'--write-media',str(OUT/'voice.mp3'),'--write-subtitles',str(OUT/'voice.srt')])
        ...
        wav=OUT/'voice.wav'; run(['espeak-ng','-v','en','-s','165','-w',str(wav),voice])
    audio=OUT/'voice.mp3' if (OUT/'voice.mp3').exists() else OUT/'voice.wav'
    if not audio.exists() or audio.stat().st_size<1000: raise RuntimeError('Voiceover generation failed.')
    ...
    for i,z in enumerate(zooms,1):
        ...
        run(['ffmpeg','-y','-loop','1','-i',str(inp),'-t',f'{sd:.3f}','-vf',vf,'-an','-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p',str(out)]); parts.append(out)
    concat=OUT/'concat.txt'; concat.write_text('\n'.join(f"file '{p.resolve()}'" for p in parts)+'\n')
    visual=OUT/'visual.mp4'; run(['ffmpeg','-y','-f','concat','-safe','0','-i',str(concat),'-c','copy',str(visual)])
    final=OUT/'reel.mp4'; sub=OUT/'voice.srt'
    vf=f"subtitles={sub.as_posix()}:force_style='FontName=DejaVu Sans,FontSize=18,PrimaryColour=&H00FFFFFF,OutlineColour=&H0018181D,BorderStyle=1,Outline=3,Shadow=1,Alignment=2,MarginV=455,MarginL=90,MarginR=190'" if sub.exists() else 'null'
    af=f"[1:a]volume=0.92[vo];aevalsrc=0.012*sin(2*PI*110*t)+0.005*sin(2*PI*220*t):s=44100:d={dur:.3f},volume=0.018[bed];[vo][bed]amix=inputs=2:duration=first:dropout_transition=2[a]"
    run(['ffmpeg','-y','-i',str(visual),'-i',str(audio),'-filter_complex',af,'-vf',vf,'-map','0:v:0','-map','[a]','-t',f'{dur:.3f}','-c:v','libx264','-preset','veryfast','-crf','19','-c:a','aac','-b:a','160k','-movflags','+faststart',str(final)])
    if not final.exists() or final.stat().st_size<100000: raise RuntimeError('Final reel was not created or is too small.')
    vdur,adur=probe(final),probe(audio)
    if abs(vdur-adur)>0.8: raise RuntimeError(f'QA failed: video={vdur:.2f}s audio={adur:.2f}s')
    (OUT/'qa.txt').write_text(f'PASS\nresolution={W}x{H}\nvideo_duration={vdur:.2f}\naudio_duration={adur:.2f}\nvoice=en-IN-NeerjaNeural\nanimated_zoompans=yes\ncaptions=lower_safe_zone\ncaption_margin_v=455\n')
